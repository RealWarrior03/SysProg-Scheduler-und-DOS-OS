...
const char * expected = "AAAAAAABBCCCCCCCC";
const char * expected_2 = "CCCCCCCCCCCCCCCC";

#define MEM_SIZE (sizeof(mem_block)*3+32)
MEMORY(MEM_SIZE, mem);
...
int main() {
	...
	my_free(ptr_a);
	 mem_print_blocks(mem);
	if(my_calloc(strlen(str_c)+1, sizeof(char), 0) != NULL) {
	...
	my_free(ptr_b);
	char * ptr_cc = my_calloc(strlen(str_c)+1, sizeof(char), 0);
	strcpy(ptr_cc, str_c);
	char * result_2 = mem_concat_data(mem);

	printf("\nExpected: %s\n", expected_2);
	printf("Found   : %s\n", result_2);

	if(strcmp(expected_2, result_2) != 0) {
		fprintf(stderr, "Did not match!\n");
		exit(1);
	}
